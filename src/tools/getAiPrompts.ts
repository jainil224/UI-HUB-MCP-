...
import type { AiPrompts } from '../types/index.js';

export const get_ai_prompts = createTool(
  'get_ai_prompts',
  'Return ready-to-use AI generation prompts (Claude, Antigravity, Lovable) for a specific UI HUB component. Requires a UI HUB Pro subscription.',
  z.object({
  ...
  async (args, user) => {
    ...
    if (!prompts) {
      return mcpError(
        'PROMPTS_NOT_FOUND',
        `No AI prompts are available for "${args.componentId}". Try get_component_code for its source instead.`
      );
    }

    const available = args.system
      ? { [args.system]: prompts[args.system as keyof AiPrompts] }
      : prompts;
    const filtered = Object.fromEntries(
      (Object.entries(available) as Array<[string, string | undefined]>).filter(([, v]) => typeof v === 'string')
    );
